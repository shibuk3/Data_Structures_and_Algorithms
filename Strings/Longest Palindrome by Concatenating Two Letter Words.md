### __Longest Palindrome by Concatenating Two Letter Words__
- [Lc Link](https://leetcode.com/problems/longest-palindrome-by-concatenating-two-letter-words/)
```md
...
public:
    ...
    unordered_map<string, int>u;int count =0;
        for(int i=0;i<words.size();i++)
        {   string x=find(words[i]);
            if(u[x]>0) {u[x]--;count=count+4;}
             else
             {u[words[i]]++;}
        ...
        {
            string x=it->first;
            if(x[0]==x[1]&&u[x]>0)  return count+2;
